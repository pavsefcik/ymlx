...
# 7. pi integration — copy the ymlx-sync extension and generate the wrapper that
#    lets it drive ymlx headlessly (bakes THIS clone's path in, so it works
#    regardless of where the repo lives).
step "Installing pi extension + wrapper…"
_YMLX_REPO_DIR="${0:A:h}"
mkdir -p ~/.pi/agent/extensions ~/.pi/agent/bin
if [[ -f "$_YMLX_REPO_DIR/extensions/ymlx-sync.ts" ]]; then
  cp -f "$_YMLX_REPO_DIR/extensions/ymlx-sync.ts" ~/.pi/agent/extensions/ymlx-sync.ts
  says "extension -> ~/.pi/agent/extensions/ymlx-sync.ts"
else
  says "SKIP: extensions/ymlx-sync.ts not found — is this a full clone of the repo?"
fi
cat > ~/.pi/agent/bin/ymlx <<EOF_YMLX
#!/usr/bin/env bash
# Generated by ymlx/install.zsh — points at ymlx.zsh in this clone.
set -euo pipefail
YMLX_ZSH="$_YMLX_REPO_DIR/ymlx.zsh" exec zsh "\$YMLX_ZSH" "\$@"
EOF_YMLX
chmod +x ~/.pi/agent/bin/ymlx
says "wrapper -> ~/.pi/agent/bin/ymlx (points at $_YMLX_REPO_DIR/ymlx.zsh)"
if command -v pi >/dev/null 2>&1; then
  ...
  says "(then either re-run this script, or let pi's /ymlx-setup command do the wiring)"
fi

print "\nDone. Add to ~/.zshrc and run:"
print "  source /path/to/ymlx/ymlx-launcher.zsh"
print "  ymlx"
print "In pi, the ymlx-sync extension is already installed — run /reload, then /ymlx-sync."